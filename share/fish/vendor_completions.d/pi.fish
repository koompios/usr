...
set -l pispecific '__fish_contains_opt -s Y pi'
...
complete -c $progname -s Y -f -l pi -n $noopt -d 'pi specific operations'
...
complete -c $progname -n "not $noopt" -l save -d 'Save current arguments to pi permanent configuration' -f
...
# pi options
complete -c $progname -n $pispecific -s c -l clean -d 'Remove unneeded dependencies' -f
complete -c $progname -n $pispecific -l gendb -d 'Generate development package DB' -f

# Show options
complete -c $progname -n $show -s d -l defaultconfig -d 'Print default pi configuration' -f
complete -c $progname -n $show -s g -l currentconfig -d 'Print current pi configuration' -f
